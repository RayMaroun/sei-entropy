...
{% file src="../../.gitbook/assets/1.-react-router-starter-code.zip" caption="react-router-starter-code" %}
...
So, in summary, we're telling `npm install` to find the library package called `react-router-dom`, install the package, and save the name of our package to our file that keeps track of all the packages for our project.
...
![Home not defined](../../.gitbook/assets/home-not-defined-error.png)
...
You can put your own content to be rendered by each component if you'd like. Refer to these complete files to make sure you got everything correct:

Procedures \`\`\`js import React from 'react'; export default function Procedures\(\) { return \(
...
Contact \`\`\`js import React from 'react'; export default function Contact\(\) { return \(
