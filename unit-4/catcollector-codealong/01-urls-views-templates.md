...
1. In our `views.py` we will now be **rendering** our template instead of sending HTTP responses, so we can update our `views.py` to comment out the import of `HttpResponse`.
